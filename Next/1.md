...
Answer: TTFB is a metric that measures the time between starting navigating to a page and when the first byte of the response begins to arrive.
...
Answer:
LCP most commonly refers to Largest Contentful Pains, a key Google web vital measuring how fast page's main content loads(aim for 2under 2.5s).
...
Answer:
HMR in Next.js refers to Hot Module Replacement, a feature known as Fast Refresh that automatically updates the running application in the browser when code is edited, without requiring a full page reload. This significantly enhances the development experience by providing a near-instantaneous feedback loop while preserving the component's local state
